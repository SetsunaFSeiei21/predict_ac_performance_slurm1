...
class GCN(Device_BaseModel):
    
    def __init__(self, feature_dim: int, hidden_dim: int, output_dim: int, dropout: float,
                embedding_layer_num: int, gcn_layer_num: int, num_heads: int, 
                decoder_layer_num: int, output_layer_num: int, performance_num: int, 
                device_messages: List[Dict[str, Any]], adj_mask: np.ndarray):
        ...
        self.register_buffer("adj_mask_tensors", torch.as_tensor(adj_mask, dtype=torch.float32))
        self.network = nn.ModuleDict({
            "embedding_layer": get_mlp_layer(
                input_dim=(feature_dim + self.device_level_one_hot_tensors.shape[1]), hidden_dim=hidden_dim,
                        output_dim=hidden_dim, dropout=dropout, layer_num=embedding_layer_num),
            "graph_conv_layer": nn.ModuleList([
                DenseGCNConv(in_channels=hidden_dim, out_channels=hidden_dim) for _ in range(gcn_layer_num)
            ]),
            "norm_layer": nn.ModuleList([nn.LayerNorm(hidden_dim) for _ in range(gcn_layer_num)]),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = torch.cat((x, final_device_level_tensors), dim=-1)
        ...
        for conv, norm in zip(self.network["graph_conv_layer"], self.network["norm_layer"]):
            ...
            embedding_tensors = conv(embedding_tensors, self.adj_mask_tensors, add_loop = False)
