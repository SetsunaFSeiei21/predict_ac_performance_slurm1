...
class DenseGATConv_NoGrad_SharedParam(nn.Module):
    """
    Shared-parameter no-grad Dense GAT.

    Compared with the old gat_no_grad_test:
    1. No extra lin_value matrix.
    2. Use one shared projection matrix.
    3. Detach score path for proxy heads.
    4. Keep value path gradient for all heads.
    ...
    This version:
        ...
        output = softmax(score(h_score)) @ h_value

    Therefore:
        proxy heads do not send score-gradient to W,
        but they still send value-gradient to W.
    """
    ...
    def __init__(
        ...
        bias: bool = True,
        true_head_ratio: float = 0.5,
        proxy_scale: float = 1.0,  # kept for interface compatibility
        freeze_proxy_attention: bool = False,
        ...
        # In this shared-param version, proxy_scale is not used.
        # It is kept only to avoid changing upper-level model interfaces.
        self.proxy_scale = float(proxy_scale)
        ...
        # One shared projection matrix, same parameter scale as normal GAT.
        self.lin = nn.Linear(
            in_features=in_channels,
            out_features=heads * out_channels,
            bias=False,
    ...
    def forward(
        ...
        # 1. Shared projection
        # ------------------------------------------------------------
        # h_value keeps value-path gradient for all heads.
        h_value = self.lin(x)
        h_value = h_value.view(B, N, self.heads, self.out_channels)

        # ------------------------------------------------------------
        # 2. Score path with head-wise detach
        # ------------------------------------------------------------
        # True heads:
        #     score-gradient is preserved.
        #
        # Proxy heads:
        #     score-gradient is cut by detach().
        #
        # This avoids adding an extra lin_value matrix.
        score_heads = []

        for h in range(self.heads):
            h_tensor = h_value[:, :, h:h + 1, :]

            if bool(self.true_head_mask[h].item()):
                score_heads.append(h_tensor)
            else:
                score_heads.append(h_tensor.detach())

        h_score = torch.cat(score_heads, dim=2)
    ...
    def _build_attention_parameters(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        If freeze_proxy_attention=True:
            proxy heads' attention vectors are also detached.
        """
...
class GAT_No_Grad_Test(Device_BaseModel):
    """
    GAT top model using DenseGATConv_NoGrad_SharedParam.
    ...
    Compared with the old gat_no_grad_test:
        old:
            lin_score + lin_value
            score-gradient truncation + value-gradient proxy

        new:
            shared lin
            score-gradient truncation only
            value path gradient preserved
    """
    ...
    def __init__(
        ...
        # In the shared-param version, proxy_scale does not affect forward/backward.
        self.proxy_scale = proxy_scale
