...
from src.layers import Decoder, Encoder_No_Grad_Test
from src.base import Device_BaseModel
from src.utils import get_mlp_layer

class ACCFormer_No_Grad_Test(Device_BaseModel):
    
    def __init__(self, feature_dim: int, hidden_dim: int, output_dim: int, dropout: float, num_heads: int,
                embedding_layer_num: int, encoder_layer_num: int, decoder_layer_num: int, output_layer_num: int, performance_num: int,
                device_messages: List[Dict[str, Any]], attn_mask: np.ndarray):
        ...
        self.register_buffer("device_level_attn_mask", (torch.tensor(attn_mask) - 1) * 1e9)
        self.network = nn.ModuleDict({
            "embedding_layer": get_mlp_layer(input_dim=(feature_dim + self.device_level_one_hot_tensors.shape[1]), hidden_dim=hidden_dim, output_dim=hidden_dim,
                                            dropout=dropout, layer_num=embedding_layer_num),
            "encoder": nn.ModuleList([
                Encoder_No_Grad_Test(input_dim=hidden_dim, hidden_dim=hidden_dim, output_dim=hidden_dim, num_heads=num_heads, dropout=dropout) for _ in range(encoder_layer_num)
                ]),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for sub_layer in self.network['encoder']:
            embedding_tensors, _ = sub_layer(embedding_tensors, self.device_level_attn_mask)
    ...
    def _initial_weight(self, freeze_direction: int, freeze_layer_num: int):
        
        assert freeze_layer_num <= (self.embedding_layer_num + self.encoder_layer_num + self.decoder_layer_num + self.output_layer_num), ("freeze_layer_num larger than all layers num!")
        ...
        for layer in modules:
            if isinstance(layer, nn.Linear):
                ...
                layer._init_weight()
                reset_num += 1
            elif isinstance(layer, Encoder_No_Grad_Test):
                layer._init_weight()
                reset_num += 1
            if reset_num >= freeze_layer_num:
                break