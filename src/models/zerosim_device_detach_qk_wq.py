...
from src.modules import Structure_Encoding_Layer_Detach_qk_wq, Parameter_Injection_Layer_Detach_qk_wq

class Zerosim_Device_Detach_qk_wq(Device_BaseModel):
    ...
    def __init__(self, feature_dim: int, hidden_dim: int, output_dim: int, dropout: float, num_heads: int, embedding_layer_num: int, structure_encoding_layer_num: int, 
        ...
        self.network = nn.ModuleDict({
            ...
            "structure_encoding_layer": nn.ModuleList([
                Structure_Encoding_Layer_Detach_qk_wq(input_dim=hidden_dim, hidden_dim=hidden_dim, output_dim=hidden_dim, dropout=dropout, num_heads=num_heads, attn_mask=attn_mask)
                for _ in range(structure_encoding_layer_num)
            ]),
            "parameter_injection_layer": nn.ModuleList([
                Parameter_Injection_Layer_Detach_qk_wq(input_dim=hidden_dim, hidden_dim=hidden_dim, output_dim=hidden_dim, dropout=dropout, num_heads=num_heads, attn_mask=self.attn_mask,
