...
from src.layers import Decoder, Encoder_No_Grad
from src.base import Device_BaseModel
from src.utils import get_mlp_layer

class ACCFormer_No_Grad(Device_BaseModel):
    ...
    def __init__(self, feature_dim: int, hidden_dim: int, output_dim: int, dropout: float, num_heads: int,
        ...
        self.network = nn.ModuleDict({
            ...
            "encoder": nn.ModuleList([
                Encoder_No_Grad(input_dim=hidden_dim, hidden_dim=hidden_dim, output_dim=hidden_dim, num_heads=num_heads, dropout=dropout) for _ in range(encoder_layer_num)
                ]),
    ...
    def _initial_weight(self, freeze_direction: int, freeze_layer_num: int):
        ...
        for layer in modules:
            if isinstance(layer, nn.Linear):
                ...
                layer._init_weight()
                reset_num += 1
            elif isinstance(layer, Encoder_No_Grad):
                layer._init_weight()
                reset_num += 1
