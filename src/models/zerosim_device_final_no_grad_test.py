...
from src.modules import Structure_Encoding_Layer_No_Grad, Parameter_Injection_Layer_No_Grad_Test

class Zerosim_Device_Final_No_Grad_Test(Device_BaseModel):
    
    """_summary_
    作为Parameter_Injection_Layer_No_Grad的测试平台
    """
    ...
    def __init__(self, feature_dim: int, hidden_dim: int, output_dim: int, dropout: float, num_heads: int, embedding_layer_num: int, structure_encoding_layer_num: int, 
        ...
        self.network = nn.ModuleDict({
            ...
            "structure_encoding_layer": nn.ModuleList([
                Structure_Encoding_Layer_No_Grad(input_dim=hidden_dim, hidden_dim=hidden_dim, output_dim=hidden_dim, dropout=dropout, num_heads=num_heads, attn_mask=attn_mask)
