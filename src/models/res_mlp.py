...
from src.layers import Decoder, ResLayer
from typing import List, Dict, Any
from src.utils import get_res_mlp_layer, get_mlp_layer

class ResMLP(Device_BaseModel):
    
    def __init__(self, feature_dim: int, hidden_dim: int, output_dim: int, dropout: float, res_block_num: int, num_heads: int, decoder_layer_num: int, 
                output_layer_num: int, performance_num: int, device_messages: List[Dict[str, Any]]):
        ...
        self.network = nn.ModuleDict({
            "embedding_layer": get_res_mlp_layer(input_dim=(feature_dim + self.device_level_one_hot_tensors.shape[1]), hidden_dim=hidden_dim, 
                                                output_dim=hidden_dim, dropout=dropout, block_num=res_block_num),
    ...
    def _initial_weight(self, freeze_direction: int, freeze_layer_num: int) -> None:
        ...
        assert freeze_layer_num <= (self.res_block_num + self.decoder_layer_num + self.output_layer_num), ("freeze_layer_num larger than all layers num!")
        ...
        for layer in modules:
            if isinstance(layer, nn.Linear):
                ...
                layer._init_weight()
                reset_count += 1
            elif isinstance(layer, ResLayer):
                layer._init_weight()
                reset_count += 1
