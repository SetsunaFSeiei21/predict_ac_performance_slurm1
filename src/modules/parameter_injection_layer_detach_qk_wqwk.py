...
from src.layers import EncoderDetachQKFreezeQK, Cross_Attention

class Parameter_Injection_Layer_Detach_qk_wqwk(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int, dropout: float, num_heads: int, attn_mask: torch.Tensor, pr_attn_mask: torch.Tensor, **kwargs) -> None:
        ...
        self.context_enhancing = EncoderDetachQKFreezeQK(input_dim=hidden_dim, hidden_dim=hidden_dim, output_dim=hidden_dim, num_heads=num_heads, dropout=dropout)
