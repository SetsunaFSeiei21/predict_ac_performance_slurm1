...
from src.layers import Encoder, Cross_Attention

class Parameter_Injection_Layer(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int, dropout: float, num_heads: int, attn_mask: torch.Tensor, pr_attn_mask: torch.Tensor, **kwargs) -> None:
        ...
        super().__init__()
        self.structure_refining = Encoder(input_dim=input_dim, hidden_dim=hidden_dim, output_dim=hidden_dim, num_heads=num_heads, dropout=dropout)
        ...
        self.context_enhancing = Encoder(input_dim=hidden_dim, hidden_dim=hidden_dim, output_dim=hidden_dim, num_heads=num_heads, dropout=dropout)
