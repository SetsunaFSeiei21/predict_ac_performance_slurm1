...
from src.layers import Encoder_No_Grad_Test, Cross_Attention

class Parameter_Injection_Layer_No_Grad_Test(nn.Module):
    
    """_summary_
    作为Encoder_No_Grad_Test的测试平台
    """
    ...
    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int, dropout: float, num_heads: int, attn_mask: torch.Tensor, pr_attn_mask: torch.Tensor, **kwargs) -> None:
        ...
        self.context_enhancing = Encoder_No_Grad_Test(input_dim=hidden_dim, hidden_dim=hidden_dim, output_dim=hidden_dim, num_heads=num_heads, dropout=dropout)
