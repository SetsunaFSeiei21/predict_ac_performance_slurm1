...
class GlobalEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attention_mask: torch.Tensor = None) -> torch.Tensor:
        ...
        # 先尝试余弦相似度
        cosine_similarity = self._cal_cosine_similarity(delta_device_tensors, x[:, :(x.shape[1] - 1), :])
        # cosine_similarity = self.sigmoid_layer(cosine_similarity)
        cosine_similarity = (cosine_similarity + 1) / 2
        final_device_tensor = (1 - cosine_similarity) * past_device_tensor + cosine_similarity * x[:, :(x.shape[1] - 1), :]
        final_output = torch.cat((final_device_tensor, global_device_tensor.unsqueeze(1)), dim= 1)
        return final_output, attn_score
    
    def _cal_cosine_similarity(self, tensor1: torch.Tensor, tensor2: torch.Tensor) -> torch.Tensor:
        return (torch.sum(tensor1 * tensor2, dim=-1) / (torch.sqrt(torch.sum(tensor1 * tensor1, dim = -1)) * torch.sqrt(torch.sum(tensor2 * tensor2, dim=-1)) + 1e-8)).unsqueeze(-1)
