...
class _ProxyKeyAttentionFunction(torch.autograd.Function):
    """
    Optimized custom autograd function for head-wise true-gradient / proxy-gradient attention.
        ...
        grad_Wk:
            ...
            proxy heads:
                proxy-gradient from value path:
                    grad_Wk_proxy = alpha * grad_Wv

        grad_alpha:
            only true heads receive score-path alpha gradient in this optimized version.

    Main optimization:
        Score-gradient is computed only for true heads.
        Proxy heads skip:
            grad_attn
            softmax backward
            grad_score
    ...
    @staticmethod
    def _merge_true_heads(
        x: torch.Tensor,
    ) -> torch.Tensor:
        """
        [B, T, N, Dh] -> [B, N, T * Dh]

        T is the number of true-gradient heads.
        """
        B, T, N, Dh = x.shape
    ...
    def forward(
        ...
        # Manual dropout so backward can reproduce the same dropout mask.
        ...
        # Proxy heads do not need Q/K_raw for backward.
        if true_head_num > 0:
    ...
    def backward(
        ...
        # ==========================================================
        # 2. Initialize grad_Wk / grad_bk / grad_log_alpha.
        # ==========================================================
        grad_k_weight = torch.zeros_like(v_weight)
        grad_k_bias = torch.zeros(
            D,
            device=x.device,
            dtype=x.dtype,
        )
        ...
        # ==========================================================
        # 3. True heads only: compute real score-gradient.
        #    This is the main training-time optimization.
        # ==========================================================
        if true_head_num > 0:
            ...
            grad_log_alpha_true = grad_alpha_true * torch.sigmoid(
                log_alpha[:true_dim]
            )
            grad_log_alpha[:true_dim] = grad_log_alpha_true.to(
                dtype=log_alpha.dtype
            )

        # ==========================================================
        # 4. Proxy heads: no score-gradient.
        #    Directly inject proxy-gradient from grad_Wv.
        # ==========================================================
        if true_dim < D:
            alpha_proxy = alpha[true_dim:].detach()
        ...
        # Return gradients for every forward input:
        #
        # x,
        # q_weight,
        # q_bias,
...
class HeadwiseTrueGradientProxyKeyAttention(nn.Module):
    ...
    Interface:
        forward(x, attention_mask) -> (out, attn_score or None)

    Important:
        For maximum speed and clean profiling, keep need_attn_score=False.
        If need_attn_score=True, the module falls back to a slower manual attention path.
    """
    ...
    def _slow_attention_with_score(
        ...
        This path is not intended for performance profiling because returning
        attention scores itself increases memory and graph bookkeeping.
        """
    ...
    def _fast_eval_attention(
        ...
        In eval mode no proxy-gradient construction is needed.
        """
        alpha_channel = self.alpha.to(device=x.device, dtype=x.dtype)
        additive_mask = self._build_additive_attention_mask(attention_mask, x)
    ...
    def forward(
        ...
        # Returning attention score is inherently memory-heavy.
        ...
        # Eval mode uses standard fast forward.
...
class Encoder_No_Grad_Test(nn.Module):
    ...
    def forward(
        ...
        # Pre-LayerNorm + optimized custom-backward no-grad/proxy-gradient attention
